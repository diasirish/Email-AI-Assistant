                    ...
                    while not success and retries < max_retries:
                        try:
                            ...
                            if error.resp.status in [400, 404]:
                                ...
                                break  # Exit the retry loop and move to the next message
                            elif error.resp.status == 500:
                                retries += 1
                                sleep_time = wait_time * (backoff_factor ** (retries - 1))
                                sleep_time += random.uniform(0, 1)  # Add jitter
                                print(f"Server error on message {msg['id']}, retrying in {sleep_time:.2f} seconds...")
                                time.sleep(sleep_time)
                            else:
                                # For other errors, re-raise the exception
                                raise
                    if not success:
                        print(f"Failed to fetch message {msg['id']} after {max_retries} retries. Skipping.")
...
if __name__ == "__main__":
    ...
    CREDENTIALS_FILE = 'client_secret2.json'
    ...
    try:
        ...
        max_pages = None  # Change this value as needed; set to None for no limit
